...
import com.android.transmart.UI.fragments.CheckinFragment;
import com.android.transmart.UI.fragments.PlaceDetailFragment;
import com.android.transmart.UI.fragments.PlaceListFragment;
...
import com.android.transmart.receivers.NewCheckinReceiver;
...
//  protected boolean followLocationChanges = true;
  ...
  protected void getLocationAndUpdatePlaces(boolean updateWhenLocationChanges) {
    ...
    AsyncTask<Void, Void, Void> findLastLocationTask = new AsyncTask<Void, Void, Void>() {
      ...
      protected Void doInBackground(Void... params) {
        ...
        Location lastKnownLocation = lastLocationFinder.getLastBestLocation(LocationConstants.MAX_DISTANCE, 
            System.currentTimeMillis()-LocationConstants.MAX_TIME);
        ...
        updatePlaces(lastKnownLocation, LocationConstants.DEFAULT_RADIUS, false);
        return null;
      }
    };
  ...
  protected void toggleUpdatesWhenLocationChanges(boolean updateWhenLocationChanges) {   
    ...
    prefsEditor.putBoolean(LocationConstants.SP_KEY_FOLLOW_LOCATION_CHANGES, updateWhenLocationChanges);
  ...
  protected void requestLocationUpdates() {
    ...
    locationUpdateRequester.requestLocationUpdates(LocationConstants.MAX_TIME, LocationConstants.MAX_DISTANCE, criteria, locationListenerPendingIntent);
    ...
    IntentFilter intentFilter = new IntentFilter(LocationConstants.ACTIVE_LOCATION_UPDATE_PROVIDER_DISABLED);
  ...
  protected void disableLocationUpdates() {
    ...
    if (LocationConstants.DISABLE_PASSIVE_LOCATION_WHEN_USER_EXIT && isFinishing())
  ...
  protected LocationListener oneShotLastLocationUpdateListener = new LocationListener() {
    public void onLocationChanged(Location l) {
      updatePlaces(l, LocationConstants.DEFAULT_RADIUS, true);
    }
  ...
  protected void updatePlaces(Location location, int radius, boolean forceRefresh) {
    if (location != null) {
      ...
      // platform versions.
      Intent updateServiceIntent = new Intent(this, LocationConstants.SUPPORTS_ECLAIR ? EclairPlacesUpdateService.class : PlacesUpdateService.class);
      ...
      updateServiceIntent.putExtra(LocationConstants.EXTRA_KEY_RADIUS, radius);
      updateServiceIntent.putExtra(LocationConstants.EXTRA_KEY_FORCEREFRESH, forceRefresh);
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
      ...
      case R.id.refresh:
    	  ...
    	  Location lastKnownLocation = lastLocationFinder.getLastBestLocation(LocationConstants.MAX_DISTANCE, 
    	            System.currentTimeMillis()-LocationConstants.MAX_TIME);
    	  // Force an update
          updatePlaces(lastKnownLocation, LocationConstants.DEFAULT_RADIUS, true);
          return true;
      default:
