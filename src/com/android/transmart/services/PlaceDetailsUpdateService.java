...
public class PlaceDetailsUpdateService extends IntentService {
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    boolean forceCache = intent.getBooleanExtra(LocationConstants.EXTRA_KEY_FORCEREFRESH, false);
    ...
    if (!doUpdate) {
      ...
      try { 
        ...
        if (cursor.moveToFirst()) {
          if (cursor.getLong(cursor.getColumnIndex(PlaceDetailsContentProvider.KEY_LAST_UPDATE_TIME)) > System.currentTimeMillis()-LocationConstants.MAX_DETAILS_UPDATE_LATENCY)
            doUpdate = false;
        }
      }
      finally {
  ...
  protected void refreshPlaceDetails(String reference, boolean forceCache) {    
	  ...
	  try {
	    ...
	    String placesFeed = LocationConstants.PLACES_DETAIL_BASE_URI + reference + LocationConstants.PLACES_API_KEY;
