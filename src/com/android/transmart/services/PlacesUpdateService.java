...
package com.android.transmart.services;
...
import com.android.transmart.providers.PlaceDetailsContentProvider;
import com.android.transmart.providers.PlacesContentProvider;
import com.android.transmart.receivers.ConnectivityChangedReceiver;
import com.android.transmart.receivers.LocationChangedReceiver;
import com.android.transmart.receivers.PassiveLocReceiver;
...
public class PlacesUpdateService extends IntentService {  
  ...
  protected void onHandleIntent(Intent intent) {
  	...
  	boolean inBackground = prefs.getBoolean(LocationConstants.EXTRA_KEY_IN_BACKGROUND, true);
    ...
    int radius = LocationConstants.DEFAULT_RADIUS;
    ...
    if (intent.hasExtra(LocationConstants.EXTRA_KEY_LOCATION)) {
      ...
      radius = extras.getInt(LocationConstants.EXTRA_KEY_RADIUS, LocationConstants.DEFAULT_RADIUS);
    }
    ...
    if (!isConnected) {
      ...
      ComponentName passiveLocationReceiver = new ComponentName(this, PassiveLocReceiver.class);
      ...
      boolean doUpdate = intent.getBooleanExtra(LocationConstants.EXTRA_KEY_FORCEREFRESH, false);
  ...
  protected void refreshPlaces(Location location, int radius) {   
    ...
    try {
      ...
      String baseURI = LocationConstants.PLACES_LIST_BASE_URI;
      String placesFeed = baseURI + "&location=" + locationStr + "&radius=" + radius + LocationConstants.PLACES_API_KEY;
      ...
      if (responseCode == HttpURLConnection.HTTP_OK) { 
        ...
        while (eventType != XmlPullParser.END_DOCUMENT) {
         if (eventType == XmlPullParser.START_TAG && xpp.getName().equals("result")) {
           ...
           Location placeLocation = new Location(LocationConstants.CONSTRUCTED_LOCATION_PROVIDER);
        ...
        prefsEditor.putLong(LocationConstants.SP_KEY_LAST_LIST_UPDATE_LAT, (long) location.getLatitude());
        prefsEditor.putLong(LocationConstants.SP_KEY_LAST_LIST_UPDATE_LNG, (long) location.getLongitude());
        prefsEditor.putLong(LocationConstants.SP_KEY_LAST_LIST_UPDATE_TIME, System.currentTimeMillis());      
        prefsEditor.commit();
      }
      else
  ...
  protected boolean addPlace(Location currentLocation, String id, String name, String vicinity, String types, Location location, String viewport, String icon, String reference, long currentTime) {
    ...
    if ((prefetchCount < LocationConstants.PREFETCH_LIMIT) &&
        (!LocationConstants.PREFETCH_ON_WIFI_ONLY || !mobileData) &&
        (!LocationConstants.DISABLE_PREFETCH_ON_LOW_BATTERY || !lowBattery)) {
      prefetchCount++;
      ...
      updateServiceIntent.putExtra(LocationConstants.EXTRA_KEY_REFERENCE, reference);
      updateServiceIntent.putExtra(LocationConstants.EXTRA_KEY_ID, id);
  ...
  protected void removeOldLocations(Location location, int radius) {
    // Stale Detail Pages
    long minTime = System.currentTimeMillis()-LocationConstants.MAX_DETAILS_UPDATE_LATENCY;
